...
Algoritmus výpočtu súradníc v spomínanej diplomovej práci je založený na znalosti celej konfigurácie RoFIbota. Algoritmus na rekonfiguráciu popísaný v tejto kapitole je založený na tom, že žiaden modul nemá znalosť celej konfigurácie RoFIbota. Z tohto dôvodu je z algoritmu na výpočet súradníc použitá iba časť výpočtu matíc, z ktorých sa súradnice dajú získať. Zvyšok bol pozmenený nasledovným spôsobom. 

Modul s najnižšou hodnotou id pozná svoje súradnice (popísané vyššie). Následne každý modul, ktorý pozná svoje koordináty, vypočíta súradnice aj všetkým modulom, ktorý sú s ním spojený hranou. Tento výpočet dokáže vykonať každý modul, nakoľko pozná hodnoty svojich uhlov voľnosti, polohu konektorov a svoje vlastné súradnice. Následne pošle každému modulu, ktorý je s ním spojený hranou správu o tom, aké súradnice má mať daný modul. Tento výpočet a rozposlanie správ sa deje výhradne jednorázovo nakoľko pri opakovaných výpočtoch a správach by došlo k cykleniu správ. Tento proces sa opakuje až kým nemá každý z modulov znalosť o svojich súradniciach. Tento popis je tiež znázornený v algoritme \ref{algorithm:coordinations}. 

\begin{algorithm}
    \caption{Výpočet súradníc modulov RoFIbota. }
    \label{algorithm:coordinations}
    
    \DontPrintSemicolon
    \SetKwInOut{Input}{vstup}\SetKwInOut{Output}{výstup}
    \SetKwData{Id}{id}
    \Input{Konfigurácia RoFIbota.}
    \Output{Každý modul pozná svoje súradnice.}
    
    \If{\Id je najnižšie v RoFIbote}{
        inicializuj počiatočné súradnice\;
    }
    \While{existuje modul, ktorý nepozná svoje súradnice}{
        \If{modul má známe súradnice {\bf and} súradnice neboli zdieľané}{
...
\begin{algorithm}
    ...
    \Output{Každý modul je v cieľovom stave alebo v stave, ktorý vznikol rekonfigurovaním v zadanom čase.}
    
    \While{existuje modul, ktorý nie je v cieľovom stave}{
        \Modul $\leftarrow$ vyber modul\;
        \Action $\leftarrow$ vyber prístupnú akciu pre \Modul\;
...
Ostáva teda popísať spôsob výberu modulu, akcie daného modulu, a to akým spôsobom sa testuje, že je možné danú akciu bezpečne vykonať. Implementovaný algoritmus využíva na výber modulu ich hodnoty unikátneho identifikátoru. Moduly sú zoradené podľa hodnoty id od najnižšieho po najvyšší a algoritmus vyberá moduly v tomto poradí cyklicky. 

Akcie sa vyberajú tak, že sa preferujú rotácie pred prepojeniami. V prípade, že modul má každý stupeň voľnosti správne natočený, tak nasledujú odpojenia a vytváranie nových hrán sa deje ako posledné v poradí. 

Nakoľko žiaden modul nemá znalosť o celej konfigurácii RoFIbota, tak je nutné testovanie validity akcie nutné testovať pomocou vzájomnej komunikácie medzi modulmi. Moduly môžu vykonať iba rotáciu niektorého zo stupňov voľnosti, odpojenia alebo pripojenia medzi dvomi modulmi. 
...
Testovanie voči kolíziám prebieha nasledovne. $\mathcal{M}$ si zistí pozície modulov v množine $\mathcal{R}$ a na základe nich dokáže vypočítať, ako sa budú pohybovať stredy strán všetkých modulov v $\mathcal{R}$. Tieto pozície $\mathcal{M}$ pošle všetkým modulom v množine $\mathcal{S}$. Každý modul v $\mathcal{S}$ na základe vlastnej polohy v priestore dokáže určiť, či bude kolidovať s niektorým z modulov v množine $\mathcal{R}$ počas rotácie. Túto skutočnosť následne správou pošlú späť $\mathcal{M}$. Z pozbieraných údajov o prípadných kolíziách dokáže $\mathcal{M}$ vyhodnotiť, či je rotácia bez kolízií. V prípade, že áno, tak vykoná rotáciu a modulom z množiny $\mathcal{R}$ na záver rozpošle ich nové pozície v priestore. 
