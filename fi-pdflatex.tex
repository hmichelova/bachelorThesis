...
Keďže je táto práca zameraná na samotné algoritmy rekonfigurácie, tak môžeme predpokladať, že moduly dokážu komunikovať každý s každým, nakoľko je RoFIbot v každom momente rekonfigurácie spojitý. Tento predpoklad je  možné zaviesť hlavne z dôvodu toho, že problém posielania správ rieši nižšia softvérová vrstva, ktorá je už vo fyzických moduloch implementovaná a je možné ju využívať. 

\subsection{Algoritmus s úplnou znalosťou konfigurácie}
\label{sec:motionPlanningAlgo}
V počiatočnej fáze algoritmu pošle každý z modulov svoju počiatočnú a cieľovú konfiguráciu pomocou broadcastu. Tým zároveň každý modul získa stavy ostatných modulov RoFIbota. Broadcast funguje iba na spojitej časti RoFIbota, teda moduly dostanú výhradne informácie od spojitého RoFIbota. Ten je vo fyzických moduloch implementovaný na nižšej úrovni softvéru.

Aby modul mal úplnú konfiguráciu RoFIbota, je nutné získať hodnoty stupňov voľnosti $\alpha$, $\beta$ a $\gamma$ o všetkých moduloch RoFIbota a zároveň je nutné mať plnú informáciu o všetkých prepojeniach modulov. Všetky tieto údaje sú uložené v stavoch modulov. Z toho vyplýva, že po vzájomnom zdieľaní stavov medzi modulmi si je každý modul schopný reprezentovať celú počiatočnú aj cieľovú konfiguráciu RoFIbota. 

To znamená, že každý modul je schopný vypočítať celú rekonfiguráciu pomocou algoritmov implementovaných v diplomovej práci \textit{Motion Planning for the RoFI Platform} \cite{vozarovaMasterThesis}, teda postupnosť validných krokov. Cieľom je, aby každý modul vypočítal rovnakú postupnosť krokov. To znamená, že z algoritmov implementovaných v spomínanej diplomovej prácu je nutné vybrať deterministický algoritmus, ktorým je buď \textit{BFS} alebo \textit{A*} (algoritmus \textit{RRT} nie je deterministický). Podľa výsledkov v spomínanej diplomovej práci využívajú moduly algoritmus \textit{A*}. 

Pre algoritmus \textit{A*} je nutné vybrať heuristiku (pre každý modul rovnakú). Pre účely tejto práce je vybraná heuristika, ktorá počíta rozdiel v hodnotách všetkých jointov. Zároveň budú moduly počítať postupnosť krokov tak, aby sa v každom kroku udiala výhradne jedna validná akcia (ide o parameter -- maximálny počet akcii v jednom kroku -- algoritmu \textit{A*}). Tento výber je vhodný hlavne kvôli optimalizovaniu krokov rekonfigurácie, ktoré je popísané neskôr. 
...
Optimalizácia bude opäť iba heuristikou a nezaručuje nájdenie optimálneho riešenia (vzhľadom na počet krokov rekonfigurácie) paralelizácie krokov, ale zaručuje, že nájdené riešenie ostane validnou rekonfiguráciou RoFIbota. Táto optimalizácia spočíva v dvoch častiach, a to v spájaní krokov a v ich výmenách. 

Na to, aby samotná fyzická rekonfigurácia (bez výpočtov) prebehla čo najrýchlejšie, je nutné minimalizovať počet krokov rekonfigurácie, čo zabezpečuje spájanie krokov v optimalizácii. Spojenie dvoch krokov je možné iba, ak tieto kroky po sebe bezprostredne nasledujú. 
...
Spojenie krokov $i$ a $i + 1$ je možné iba v prípade ak platí, že $\mathcal{S}'_i$ je validný krok. Zároveň musí platiť, že ak zjednotenie krokov $i$ a $i + 1$ aplikujeme na konfiguráciu $\mathcal{C}_{i - 1}$, tak výsledok bude rovnaký ako keby sme ich aplikovali postupne. Čo je ekvivalentne zapísané nasledovne: $\mathcal{S}_{i + 1}$ \lstinline{<*>} $(\mathcal{S}_i$ \lstinline{<*>} $\mathcal{C}_{i - 1}) = \mathcal{S}'_i$ \lstinline{<*>} $\mathcal{C}_{i - 1}$. 
